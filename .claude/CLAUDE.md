...
- **Лекции курса — единственный корпус** для обоих списков. Вопросы курса (`kind: course`) должны покрываться лекциями почти полностью. Общие вопросы (`kind: general`) — частично: по ожиданиям владельца без пары в курсе остаются `g02, g05, g09, g10, g20–g26`. Для них особенно важны честный `coverage` и блок `outside_lectures_md`.
...
9. **(Позже)** Добавление второго курса без изменений кода; `deploy.yml` на GitHub Pages.
